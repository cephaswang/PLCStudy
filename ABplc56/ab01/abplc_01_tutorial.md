...
![Automation definition](images/ab01_01.jpg)
...
![Why automation - manual control vs automated control diagram](images/ab01_02.jpg)
...
请看这张图：一名工人（Operator）正在操作手动阀门（Hand Valve），并查看温度计（Temperature Gauge）。他的工作是控制手动阀门——工厂内有一股燃料供给，他需要控制其流量。他根据温度手动调节阀门。
...
再看右边的图：工人被替换成了**控制器（Controller）**，手动阀门变成了**控制阀（Control Valve）**，温度感应部分变成了**变送器（Transmitter）**。
...
Based on the set point and the process variable, the controller makes a decision and sends a control output (**CO**) to the control valve. The control valve then automatically opens, closes, or adjusts its position according to the controller's decision.
控制器根据设定值和过程变量做出判断，并向控制阀发出**控制输出（CO，Control Output）**信号。控制阀会根据控制器的决策自动开启、关闭或调整开度。
...
![Why automation - key benefits list](images/ab01_03.jpg)

- To Increase the Productivity / 提高生产效率
- To Improve the Product Quality / 提升产品质量
- To Reduce the Labor or Production Cost / 降低人力与生产成本
- To Improve Safety / 提高安全性
- To Assist Remote Monitoring / 便于远程监控
